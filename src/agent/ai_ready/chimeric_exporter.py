...
CHIMERIC_SCHEMA_VERSION = "chimeric_train_v0"
CHIMERIC_COLUMNS = [
    "project_accession",
    "source_file",
    "search_result_path",
    ...
    "spectrum_id",
    "component_count",
    "component_peptides_json",
    "component_modified_sequences_json",
    "component_charges_json",
    "precursor_mz",
    "best_q_value",
...
class ChimericExportResult(JsonModel):
    status: str
    schema_version: str = CHIMERIC_SCHEMA_VERSION
    output_parquet: str
    preview_csv: str
    report_json: str
    ...
    filter_counts: dict[str, int] = Field(default_factory=dict)
    warnings: list[str] = Field(default_factory=list)
    inputs: list[ChimericInput] = Field(default_factory=list)


def export_chimeric_ai_ready(
    ...
    *,
    ...
    input_reports: list[ChimericInput] = []
    ...
    combined = pd.concat(frames, ignore_index=True) if frames else pd.DataFrame(columns=CHIMERIC_COLUMNS)
    combined = combined.loc[:, CHIMERIC_COLUMNS]

    output_parquet = output_dir / "chimeric_train.parquet"
    preview_csv = output_dir / "chimeric.preview.csv"
    report_json = output_dir / "chimeric_export_report.json"
    schema_json = output_dir / "chimeric_schema.json"
    ...
    report = {
        ...
        "schema_version": CHIMERIC_SCHEMA_VERSION,
        "rows_in": rows_in,
        "rows_out": rows_out,
        ...
        "component_count_distribution": _value_distribution(combined.get("component_count")),
        "search_result_count": len(input_reports),
        "inputs": [item.model_dump(mode="json") for item in input_reports],
        "outputs": {
            "chimeric_train_parquet": str(output_parquet),
            "preview_csv": str(preview_csv),
            "schema_json": str(schema_json),
        },
    }
    write_json(report_json, report)
    return ChimericExportResult(
        status=export_status,
...
def _load_one_result(
    ...
    *,
...
) -> tuple[pd.DataFrame, ChimericInput]:
    if not path.exists():
    ...
    if missing:
        ...
        return pd.DataFrame(columns=CHIMERIC_COLUMNS), report
    ...
    output["instrument_family"] = [source_metadata[str(source)][0].get("instrument_family") or "" for source in output["source_file"]]
    ...
    if valid_rows:
        ...
        for spectrum_id, group in valid_frame.groupby("spectrum_id", sort=True):
            component_peptides = _ordered_unique(group["peptide_sequence"].tolist())
            if len(component_peptides) < 2:
                filter_counts["no_multi_peptide_assignment"] += int(len(group))
                continue
            spectrum = _match_spectrum(str(spectrum_id), spectra)
            if spectrum is None:
                filter_counts["spectrum_not_matched"] += int(len(group))
                warnings.append("spectrum_not_matched")
                continue
            first = group.iloc[0].to_dict()
            rows.append(
                {
                    "project_accession": first.get("project_accession", ""),
                    "source_file": first.get("source_file", ""),
                    "search_result_path": str(path),
                    "peaklist_path": spectrum.path,
                    "spectrum_id": spectrum_id,
                    "component_count": len(component_peptides),
                    "component_peptides_json": json.dumps(component_peptides, ensure_ascii=False),
                    "component_modified_sequences_json": json.dumps(_ordered_unique(group["modified_sequence"].tolist()), ensure_ascii=False),
                    "component_charges_json": json.dumps(_ordered_unique([_clean_number(item) for item in group["charge"].tolist()]), ensure_ascii=False),
                    "precursor_mz": spectrum.precursor_mz,
                    "best_q_value": _safe_min(group["q_value"]),
                    "best_psm_probability": _safe_max(group["psm_probability"]),
                    "search_engine": first.get("search_engine", ""),
                    "species": first.get("species", ""),
                    "instrument_family": first.get("instrument_family", ""),
                    "fragmentation_method": first.get("fragmentation_method", ""),
                    "ptm_type": first.get("ptm_type", ""),
                    ...
                    "label_source": "multi_peptide_psm_tsv_plus_mgf",
                }
            )

    if not rows:
        warnings.append("no_multi_peptide_assignment")
    filtered = pd.DataFrame(rows, columns=CHIMERIC_COLUMNS)
    report = ChimericInput(
        path=str(path),
        rows_in=int(len(frame)),
...
def _ordered_unique(values: list[Any]) -> list[Any]:
    ...
    for value in values:
        ...
        if key in seen:
            continue
        seen.add(key)
        result.append(value if not isinstance(value, float) else _clean_number(value))
    return result


def _clean_number(value: Any) -> int | float | str:
    try:
        number = float(value)
...
def _schema_payload() -> dict[str, Any]:
    return {
        "schema_version": CHIMERIC_SCHEMA_VERSION,
        "target_schema": "chimeric_train.parquet",
        "columns": {
            "component_peptides_json": "JSON array of unique peptide components assigned to the same spectrum.",
            "component_modified_sequences_json": "JSON array of modified peptide component labels when available.",
            "component_charges_json": "JSON array of component charge states from source PSM rows.",
            "spectrum_mz_json": "JSON array of MGF peak m/z values.",
            "spectrum_intensity_json": "JSON array of MGF peak intensities.",
            "label_source": "multi_peptide_psm_tsv_plus_mgf for v0 rows.",
        },
        "notes": [
            "v0 exports only conservative multi-peptide spectrum labels already present in search results.",
            "v0 does not run chimeric search and does not infer components from single-peptide PSMs.",
            "If no spectrum has two or more distinct peptide assignments, rows_out is 0 and no labels are fabricated.",
        ],
    }
