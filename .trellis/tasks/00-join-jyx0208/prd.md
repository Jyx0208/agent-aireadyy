...
`Jyx0208` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/Jyx0208/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/Jyx0208/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python ./.trellis/scripts/task.py list --assignee Jyx0208` to
...
python ./.trellis/scripts/task.py archive 00-join-jyx0208
```
